...
'use client';
/**
 * @fileOverview Home page for the FashionFlow AI application.
 * Provides an introduction to the application, its features, and navigation links.
 * Displays user authentication status and allows login/logout.
 */
...
/**
 * Home component.
 * Renders the main landing page content for the "/" route.
 * @returns {JSX.Element} The rendered Home page.
 */
export default function Home() {
  const { user, loading, logOut } = useAuth(); // Authentication context hook
    ...
    <div className="flex min-h-screen flex-col bg-gradient-to-br from-background via-secondary/30 to-background dark:from-background dark:via-secondary/10 dark:to-background text-foreground">
      ...
      <header className="sticky top-0 z-50 flex items-center justify-between p-3 sm:p-4 bg-card/80 dark:bg-card/70 backdrop-blur-lg shadow-md border-b border-border/40">
        ...
        <nav className="flex items-center gap-1 sm:gap-2">
          ...
          {!loading && user ? (
            // If user is logged in, show My Account dropdown with logout
            <DropdownMenu>
            ...
            // If user is not logged in and auth is not loading, show Login button
