...
import {BrainCircuit, TrendingUp, BarChartBig, Info, UploadCloud} from 'lucide-react'; // Removed Activity, Added UploadCloud
...
export default function Home() {
  ...
  const [hasUploadedAndDetailed, setHasUploadedAndDetailed] = useState(false); // Track if initial upload & details happened

  const handleTrendPrediction = useCallback(async () => {
    // Guard clause: only predict if all required data is present
    if (!productDescription || !location || !ageSuitability || !gender) {
      // Optionally show a toast or message if prediction is attempted prematurely
      // But primarily rely on the useEffect guard
      return;
    }

    setIsPredicting(true);
    setTrendPrediction(null); // Clear previous prediction
    ...
    try {
      ...
      setHasUploadedAndDetailed(true); // Mark that a successful prediction cycle has occurred
    }
  }, [productDescription, location, ageSuitability, gender]); // Removed hasUploadedAndDetailed dependency to avoid loop

  // Function to generate somewhat realistic sales data based on analysis
  const generateSalesData = (
    prediction: string, // Use predictedTrend directly for simpler logic or combined analysis
    loc: string,
    age: string,
    gen: string
  ): Array<{name: string; sales: number}> => {
    let baseSales = 1500; // Increased base sales
    const today = new Date();
    const modifier = calculateSalesModifier(prediction, loc, age, gen);
    const trendMultiplier = getTrendMultiplier(prediction);
    const seasonality = [0.9, 1.0, 1.15, 1.25, 1.1]; // Slightly stronger seasonality
    ...
    const simulatedData = Array.from({length: 5}, (_, i) => {
      ...
      // Apply modifier, trend, seasonality, and some randomness
      const monthSales = baseSales * modifier * Math.pow(trendMultiplier, i) * seasonality[i] * (1 + (Math.random() * 0.15 - 0.075)); // +/- 7.5% randomness
      return {
        name: `${monthName} ' ${String(nextMonth.getFullYear()).slice(-2)}`, // e.g., "Apr '25"
        sales: Math.max(50, Math.round(monthSales)), // Ensure sales are not unrealistically low
      };
    });
  ...
  // Calculates a modifier based on various factors
  const calculateSalesModifier = (prediction: string, loc: string, age: string, gen: string): number => {
    ...
    // Location adjustments (more nuanced)
    if (['colombo', 'galle fort', 'kandy city', 'negombo'].some(city => locLower.includes(city)) || locLower.includes('urban')) modifier += 0.2;
    else if (locLower.includes('suburban') || locLower.includes('major town')) modifier += 0.05;
    else if (locLower.includes('rural') || locLower.includes('village')) modifier -= 0.15;

    // Age adjustments (more nuanced)
    if (['teen', '15-19', 'young adult', '18-25', '20s'].some(term => ageLower.includes(term))) modifier += 0.25;
    else if (['25-35', '30s', 'millennial'].some(term => ageLower.includes(term))) modifier += 0.1;
    else if (['35-50', '40s', 'middle-aged'].some(term => ageLower.includes(term))) modifier -= 0.05;
    else if (['50+', 'senior', 'older'].some(term => ageLower.includes(term))) modifier -= 0.2;

    // Gender adjustments
    if (genLower === 'unisex') modifier += 0.1;
    else if (genLower.includes('women') || genLower.includes('female')) modifier += 0.05; // Slightly larger market generally
    else if (genLower.includes('men') || genLower.includes('male')) modifier -= 0.02; // Slightly smaller or more competitive

    return Math.max(0.15, modifier); // Ensure modifier doesn't go below 0.15
  };

  // Determines a growth/decline multiplier per month based on the trend
  const getTrendMultiplier = (prediction: string): number => {
    const predictionLower = prediction.toLowerCase();
    if (predictionLower.includes('strong growth') || predictionLower.includes('high demand') || predictionLower.includes('significant renewal')) return 1.18; // 18% growth month-over-month
    if (predictionLower.includes('moderate growth') || predictionLower.includes('growing popularity') || predictionLower.includes('likely renewal')) return 1.08; // 8% growth
    if (predictionLower.includes('stable') || predictionLower.includes('potential renewal')) return 1.03; // 3% growth
    if (predictionLower.includes('niche') || predictionLower.includes('emerging')) return 1.01; // 1% growth
    if (predictionLower.includes('decline') || predictionLower.includes('low demand') || predictionLower.includes('unlikely renewal')) return 0.92; // 8% decline
    return 1.0; // Default stable
  };

  // Trigger prediction only when all required fields are filled *after* the initial upload/details cycle
  useEffect(() => {
    // Only trigger if all fields have values AND the initial setup is complete
    if (productDescription && location && ageSuitability && gender && hasUploadedAndDetailed) {
      handleTrendPrediction();
    }
    // Intentionally excluding handleTrendPrediction from deps to avoid re-triggering on its change
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [productDescription, location, ageSuitability, gender, hasUploadedAndDetailed]);

  const onUploadAndDetailsComplete = useCallback(() => {
     setHasUploadedAndDetailed(true); // Set flag to true, which will trigger the useEffect
  }, []);
      ...
      <div className="flex min-h-screen flex-col bg-gradient-to-br from-background via-secondary/30 to-background dark:from-background dark:via-secondary/10 dark:to-background text-foreground">
        <header className="sticky top-0 z-10 flex items-center justify-between p-3 sm:p-4 bg-card/80 dark:bg-card/70 backdrop-blur-lg shadow-md border-b border-border/40">
          <div className="flex items-center gap-2 sm:gap-3">
             {/* Use a placeholder SVG or simple div for the logo area if Activity icon is problematic */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 xl:gap-8">
            ...
            <div className="lg:col-span-1">
              <ProductUploadForm
                ...
                onUploadComplete={onUploadAndDetailsComplete} // Trigger prediction readiness
              />
            </div>

            {/* Column 2 & 3: Results */}
            <div className="lg:col-span-2 grid grid-cols-1 gap-4 md:gap-6 xl:gap-8">
              {/* Only show results section if prediction is happening or has happened */}
              {(isPredicting || hasUploadedAndDetailed) && (
                <>
                  {/* Prediction Card */}
                   <Card className="shadow-lg border border-border/50 overflow-hidden bg-card/90 dark:bg-card/80 rounded-xl backdrop-blur-sm transition-all duration-300 hover:shadow-xl hover:border-primary/30">
                    ...
                    <CardContent className="p-4 sm:p-6 min-h-[60px]">
                      {isPredicting ? (
                        ...
                        <p className="text-lg sm:text-xl font-semibold text-foreground leading-snug">{trendPrediction}</p>
                         ...
                         <p className="text-muted-foreground italic">Waiting for prediction...</p>
                      )}
                    </CardContent>
                  </Card>
                   ...
                   <Card className="shadow-lg border border-border/50 overflow-hidden bg-card/90 dark:bg-card/80 rounded-xl backdrop-blur-sm transition-all duration-300 hover:shadow-xl hover:border-primary/30">
                    ...
                    <CardContent className="p-4 sm:p-6 min-h-[100px]">
                      {isPredicting ? (
                        ...
                        <p className="text-foreground/90 leading-relaxed whitespace-pre-wrap text-sm sm:text-base">{trendAnalysis}</p> // Use pre-wrap for formatting
                      ) : (
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                          <LineChart data={chartData} margin={{top: 5, right: 25, left: -15, bottom: 5}}>
                            ...
                            <YAxis
                              ...
                              domain={['auto', 'auto']} // Let recharts calculate domain
                              allowDecimals={false}
                            />
                            <Tooltip
                              ...
                              formatter={(value: number) => [`${value.toLocaleString()} units`, "Est. Sales"]} // Format tooltip content
                            />
                            ...
                            <Line
                              ...
                              fillOpacity={1} // Enable fill
                              fill="url(#salesGradient)" // Apply gradient fill
                            />
                          </LineChart>
                        </ResponsiveContainer>
                      ) : (
                           <p className="text-muted-foreground italic text-center py-10">Sales trend chart will appear here.</p>
                      )}
                    </CardContent>
                  </Card>
                </>
              )}
                 {/* Placeholder when no upload has happened */}
              {!isPredicting && !hasUploadedAndDetailed && (
                <Card className="shadow-lg border border-dashed border-border/40 bg-gradient-to-br from-muted/20 via-transparent to-muted/10 dark:from-muted/10 dark:via-transparent dark:to-muted/5 rounded-xl flex items-center justify-center min-h-[300px] lg:min-h-[500px] transition-all duration-500 hover:shadow-primary/10 hover:border-primary/30">
                    <div className="text-center p-8 opacity-70 group-hover:opacity-100 transition-opacity">
                        ...
                        <p className="text-sm text-muted-foreground/80 mt-1">Fill in the details on the left to generate insights and predict market trends.</p>
                    </div>
                </Card>
              )}
            </div>
