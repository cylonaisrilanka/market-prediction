...
import {BrainCircuit, TrendingUp, BarChartBig, Info, UploadCloud, Home as HomeIcon} from 'lucide-react'; // Added HomeIcon
...
export default function PredictPage() { // Renamed component
  ...
  }, [productDescription, location, ageSuitability, gender]);
  ...
  // Function to generate somewhat realistic sales data based on analysis
    const generateSalesData = (
        ...
        const seasonality = [0.9, 1.0, 1.15, 1.25, 1.1]; // Example seasonality
        ...
        const simulatedData = Array.from({length: 5}, (_, i) => {
            ...
            const monthSales = baseSales * modifier * Math.pow(trendMultiplier, i) * seasonality[i] * (1 + (Math.random() * 0.15 - 0.075));
            return {
                name: `${monthName} ' ${String(nextMonth.getFullYear()).slice(-2)}`,
                sales: Math.max(50, Math.round(monthSales)),
            };
        });
    ...
    const calculateSalesModifier = (prediction: string, loc: string, age: string, gen: string): number => {
        ...
        else if (genLower.includes('men') || genLower.includes('male')) modifier -= 0.03;

        // Interaction effects (example: Trendy + Young Adult + Urban = higher boost)
        if ((predictionLower.includes('growth') || predictionLower.includes('popular') || predictionLower.includes('renewal')) &&
         ...
         // Interaction effects (example: Niche + Older + Rural = lower penalty or slight boost if specific)
         if ((predictionLower.includes('niche')) &&
             (['50+', 'senior', 'older'].some(term => ageLower.includes(term))) &&
             (locLower.includes('rural'))) {
             modifier += 0.05; // Could be a specific niche appealing to this group
         }


        return Math.max(0.1, modifier); // Ensure modifier doesn't go below 0.1
    };
  ...
  }, [productDescription, location, ageSuitability, gender, hasUploadedAndDetailed]); // Correct dependencies

  // This function is called by ProductUploadForm when the image is processed and details are entered
  const onUploadAndDetailsComplete = useCallback(() => {
     setHasUploadedAndDetailed(true); // Set flag to true
     // No need to call handleTrendPrediction here, useEffect will handle it
  }, []);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 xl:gap-8">
            ...
            <div className="lg:col-span-1">
              <ProductUploadForm
                ...
                onUploadComplete={onUploadAndDetailsComplete} // Pass the callback
              />
            </div>
            ...
            <div className="lg:col-span-2 grid grid-cols-1 gap-4 md:gap-6 xl:gap-8">
              ...
              {!isPredicting && !hasUploadedAndDetailed && (
                <Card className="shadow-lg border border-dashed border-border/40 bg-gradient-to-br from-muted/20 via-transparent to-muted/10 dark:from-muted/10 dark:via-transparent dark:to-muted/5 rounded-xl flex flex-col items-center justify-center min-h-[300px] lg:min-h-[500px] transition-all duration-500 hover:shadow-primary/10 hover:border-primary/30">
                    <div className="text-center p-8 opacity-70 group-hover:opacity-100 transition-opacity">
                        ...
                        <p className="text-sm text-muted-foreground/80 mt-1 max-w-xs mx-auto">Fill in the image and details on the left to generate insights and predict market trends.</p>
                    </div>
                </Card>
              )}
            </div>
